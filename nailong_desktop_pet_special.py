...
class NailongPet:
    ...
    def _build_menu(self):
        ...
        self.menu.add_command(label="  我才是奶龙", state="disabled")
        self.menu.add_command(label="  奶龙>47cat", state="disabled")
        self.menu.add_separator()
        self.menu.add_command(label="  跳舞", command=lambda: self._force_state(self.ST_DANCE))
        self.menu.add_command(label="  跑步", command=self._force_walk)
        ...
        self.menu.add_command(label="  黄桃罐头卖萌", command=lambda: self._force_state(self.ST_WAIT))
        self.menu.add_separator()
        self.menu.add_command(label="  猫猫阿姨失败后虐待奶龙次数: 0", state="disabled")
    ...
    def _update_pet_menu(self):
        try:
            self.menu.entryconfigure(
                self._pet_menu_idx, label=f"  猫猫阿姨失败后虐待奶龙次数: {self.pet_count}"
            )
        except Exception:
            pass
